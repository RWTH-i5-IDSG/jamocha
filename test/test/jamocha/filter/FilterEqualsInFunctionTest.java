	...
	public void testFunctionWithArgumentsCompositeEqualsInFunctionTrueEquality() {
		...
		f =
				new PredicateBuilder(equals)
						...
						.addFunction(new FunctionBuilder(minus).addDouble(1337.).addPath(p3, a3).build()).buildPFE();
		g =
				new PredicateBuilder(equals)
						.addFunction(new FunctionBuilder(plus).addPath(p1, a1).addPath(p2, a2).build())
						.addFunction(new FunctionBuilder(minus).addDouble(1337.).addPath(p3, a3).build()).buildPFE();
	...
	public void testFunctionWithArgumentsCompositeEqualsInFunctionTrueNormalize() {
		...
		g =
				new PredicateBuilder(equals)
						...
						.addFunction(new FunctionBuilder(plus).addPath(p1, a1).addPath(p2, a2).build()).buildPFE();
		pf = new PathFilter(f);
		pg = new PathFilter(g);
		assertTrue(FilterFunctionCompare.equals(
				PathFilterToAddressFilterTranslator.translate(pf, counterColumnMatcherMockup),
		...
		g =
				new PredicateBuilder(equals)
						.addFunction(new FunctionBuilder(minus).addDouble(1337.).addPath(p3, a3).build())
						.addFunction(new FunctionBuilder(plus).addPath(p2, a2).addPath(p1, a1).build()).buildPFE();
		pg = new PathFilter(g);
		assertTrue(FilterFunctionCompare.equals(
				PathFilterToAddressFilterTranslator.translate(pf, counterColumnMatcherMockup),
				PathFilterToAddressFilterTranslator.translate(pg, counterColumnMatcherMockup).getNormalisedVersion()));
	...
	public void testFilterEqualsInFunction() {
		...
		h =
				new PredicateBuilder(lessL).addLong(50000)
						.addFunction(new FunctionBuilder(plusL).addPath(p2, a3).addPath(p3, a4).build()).buildPFE();
