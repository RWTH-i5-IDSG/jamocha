...
public class Loader {
	...
	/**
	 * Recursively loads all class files in the specified directory. The name of the directory is to
	 * be split up into the part specifying the path to the start of the package hierarchy and the
	 * part specifying which part of the hierarchy is to be loaded. <br />
	 * <b>Example:</b>
	 * loadClassesInDirectory("target/classes","org/jamocha/function/impls/predicates"); loads all
	 * class files within "target/classes/org/jamocha/function/impls/predicates" and the files found
	 * in that folder there have the package specification "org.jamocha.function.impls.predicates".
	 * Class files in sub directories of the predicates folder have their package specification
	...
	public static void loadClassesInDirectory(final String pathToDir, final String packageString) {
		...
		try (final Stream<Path> files =
				java.nio.file.Files.walk(FileSystems.getDefault().getPath(pathToDir, packageString))) {
