...
/**
 * Translates SL code of a propose to CLIPS code. A propose contains an action
 * and a proposition, which specifies the preconditions for performing the
 * action. The action will be performed by the sender, if the receiver accepts
 * the preconditions.
 * 
 * @author Markus Kucay, Alexander Wilden
 * 
 */
class Propose extends SLPerformativeTranslator {
	...
	public String getCLIPS(String slContent) throws AdapterTranslationException {
		...
		result
				.append("(assert (agent-propose-result (message %MSG%)(action \"");
