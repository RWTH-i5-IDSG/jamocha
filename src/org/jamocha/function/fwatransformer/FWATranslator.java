...
@Getter
public abstract class FWATranslator<S extends ExchangeableLeaf<S>, T extends ExchangeableLeaf<T>> implements
		FunctionWithArgumentsVisitor<S> {
	...
	public void visit(final Assert<S> fwa) {
		this.functionWithArguments =
				new Assert<>(fwa.getNetwork(), (Assert.TemplateContainer<T>[]) translateArgs(fwa.getArgs(),
	...
	@SuppressWarnings("unchecked")
	private <A extends FunctionWithArguments<?>> A[] translateArgs(final A[] originalArgs, final IntFunction<A[]> array) {
