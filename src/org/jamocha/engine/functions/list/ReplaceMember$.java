 ...
 * 
 * Replaces specific items in a list with a given replacement. Arguments can
 * either be single items or lists of items. If a list is given to be replaced,
 * the items are only replaced if they appear in identical (correct and
 * coherent) order in the first list. The function walks through the first list
 * and tries to match each position to any of the arguments that are to be
 * replaced. If it finds a match it replaces the item(s) at the current
 * position. Afterwards it moves on to the next position after the replacement
 * and tries to match this position anew to any of the arguments that are to
 * replaced. Therefore the arguments are not necessarily processed in the order
 * they are given, but replacements will not be matched recursively. Returns a
 * new list consisting of untouched and replaced items.
 */
			...
			if (subject.is(JamochaType.LIST)) {
				...
				while (startIndex < end) {
					...
					searchFor: for (JamochaValue searchValue : searchValues)
						...
						} else // found something. add it to the temp list
