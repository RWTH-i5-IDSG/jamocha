...
public class VariableValueContext {
    ...
    /**
     * Initializes the variables of the left hand side using the initializers registered via {@link
     * #addInitializer(Symbol, SlotInFactAddress)}.
     *
     * @param token
    ...
    public void initialize(final AssertOrRetract<?> token) {
        ...
        for (final Entry<Symbol, Either<SlotInFactAddress, Object>> entry : initializers.entrySet()) {
            ...
            variableValues.put(key,
                    either.fold(address -> token.getValue(address.getFactAddress(), address.getSlotAddress()),
                            value -> value));
        }
    }
}
