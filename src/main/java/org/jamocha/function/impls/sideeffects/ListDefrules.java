 ...
 */
public abstract class ListDefrules implements Function<Object> {
    public static final String IN_CLIPS = "list-defrules";
    ...
    static {
        FunctionDictionary.addFixedArgsGeneratorWithSideEffects(IN_CLIPS, SlotType.EMPTY,
                (final SideEffectFunctionToNetwork network, final SlotType[] paramTypes) -> {
                    return new ListDefrules() {
                        @Override
                        public Object evaluate(final Function<?>... params) {
                            network.getLogFormatter().messageRuleList(network);
                            return null;
                        }
                    };
                });
