...
public class SamePathsFilterCombiningOptimizer implements Optimizer {
    ...
    static class Identifier implements PathFilterListVisitor {
        ...
        public void visit(final PathNodeFilterSet filterSet) {
            ...
            final Map<HashSet<Path>, List<PathFilter>> map = filters.stream()
                    .collect(groupingBy(pnfs -> PathCollector.newHashSet().collect(pnfs).getPaths(), toList()));
            for (final List<PathFilter> set : map.values()) {
                ...
                filters.add(
                        new PathFilter(GenericWithArgumentsComposite.newPredicateInstance(And.IN_CLIPS, arguments)));
            }
        }
