...
public class TrivialPathSetToPathListConverter implements PathFilterSetVisitor {
    ...
    public void visit(final PathExistentialSet set) {
        ...
        if (closurePaths.isEmpty()) {
            final PathLeaf[] args = (regularPaths.isEmpty() ? new PathLeaf[]{new PathLeaf(set.getInitialPath(), null)}
                    : toArray(regularPaths.stream().map(p -> new PathLeaf(p, null)), PathLeaf[]::new));
            existentialClosure = new PathFilter(GenericWithArgumentsComposite
                    .newPredicateInstance(And.IN_CLIPS, set.existentialClosure.getFunction(),
                            new PredicateWithArgumentsComposite<PathLeaf>(DummyPredicate.INSTANCE, args)));
        }
        this.result = new PathExistentialList(set.getInitialPath(), PathFilterList
                .toSimpleList(purePart.stream().map(TrivialPathSetToPathListConverter::convert).collect(toList())),
                PathNodeFilterSet
                        .newExistentialPathNodeFilterSet(!set.isPositive(), existentialPaths, existentialClosure));
    }
