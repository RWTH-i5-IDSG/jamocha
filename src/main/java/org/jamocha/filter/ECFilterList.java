...
public interface ECFilterList extends Visitable<ECFilterListVisitor> {
    ...
    class ECNodeFilterSet extends NodeFilterSet<ECLeaf, ECFilter> implements ECFilterList {
    ...
    class ECExistentialList implements ECFilterList {
    ...
    static ECFilterList toSimpleList(final List<ECFilterList> list) {
    ...
    class ECSharedListWrapper {
        ...
        public void addSharedColumn(final Map<ECSharedList, ECFilterList> filters) {
            for (final ECSharedList sibling : this.sharedSiblings) {
        ...
        public class ECSharedList implements ECFilterList {
            ...
            public ImmutableList<ECSharedList> getSiblings() {
                return ECSharedListWrapper.this.sharedSiblings;
            }
            ...
            private abstract class Proxy {
                final ImmutableMap<ECSharedList, ImmutableList<ECFilterList>> siblingToFilters =
                        Maps.toMap(ECSharedListWrapper.this.sharedSiblings,
                                ECSharedList::getUnmodifiableFilterListCopy);
                final ImmutableMap<ECFilterList, Integer> chosenElementToIndex;

                private Proxy() {
                    final ImmutableList<ECFilterList> chosenElements = this.siblingToFilters.get(ECSharedList.this);
                    this.chosenElementToIndex =
            ...
            public class ModificationProxy extends Proxy {
                ...
                public void add(final ECFilterList oldFilter) {
                    final int index = this.chosenElementToIndex.get(oldFilter);
                    ECSharedListWrapper.this
                            .addSharedColumn(Maps.transformValues(this.siblingToFilters, l -> l.get(index)));
                }

                public void addAll(final Iterable<? extends ECFilterList> oldFilters) {
                    final List<Integer> indices =
                            Lambdas.stream(oldFilters).map(this.chosenElementToIndex::get).collect(toList());
                    final ImmutableMap<ECSharedList, Iterable<ECFilterList>> oldFilterMap =
                            Maps.toMap(ECSharedListWrapper.this.sharedSiblings, (final ECSharedList sibling) -> {
                                final ImmutableList<ECFilterList> siblingFilters = this.siblingToFilters.get(sibling);
