...
public class MapExtender<K, V> extends LazyMap<K, V> {
    ...
    private final Set<K> keySet = SetExtender.with(this.wrapped.keySet(), this.additionalEntry.getKey());
    @Getter(lazy = true, value = AccessLevel.PRIVATE)
    private final CollectionExtender<V> values =
            CollectionExtender.with(this.wrapped.values(), this.additionalEntry.getValue());
    @Getter(lazy = true, value = AccessLevel.PRIVATE)
    private final Set<Map.Entry<K, V>> entrySet = SetExtender.with(this.wrapped.entrySet(), this.additionalEntry);
    @Getter(lazy = true, value = AccessLevel.PRIVATE)
    private final int size = this.wrapped.size() + 1;

    public static <K, V> MapExtender<K, V> with(final Map<K, V> toWrap, final Entry<K, V> additionalEntry) {
        ...
        return new MapExtender<>(toWrap, additionalEntry);
    }

    public static <K, V> MapExtender<K, V> with(final Map<K, V> toWrap, final K additionalKey,
            final V additionalValue) {
        return with(toWrap, Pair.of(additionalKey, additionalValue));
    }

    public MapExtender<K, V> with(final Entry<K, V> additionalEntry) {
        return with(this, additionalEntry);
    }

    public MapExtender<K, V> with(final K additionalKey, final V additionalValue) {
