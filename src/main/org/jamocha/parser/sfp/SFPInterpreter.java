...
import org.jamocha.rete.configurations.BooleanConditionConfiguration;
...
import org.jamocha.rete.configurations.BooleanConditionConfiguration.BoolOperator;
...
public class SFPInterpreter implements SFPParserVisitor {
	...
	for (int i = 1; i < node.jjtGetNumChildren(); i++) {
	    slots[i - 1] = (SlotConfiguration) node.jjtGetChild(i).jjtAccept(this, data);
	}
	...
	ConditionConfiguration[] conditionList = new ConditionConfiguration[node
		...
		- j];
	for (int i = j; i < k; i++) {
	    conditionList[i - j] = (ConditionConfiguration) (node.jjtGetChild(i).jjtAccept(this,
		    data));
	}
	...
	BooleanConditionConfiguration boolConf = new BooleanConditionConfiguration();
	...
	boolConf.setNestedCCs(ccs);
	boolConf.setBoolOperator(BoolOperator.NOT);
	return boolConf;
    }
	...
	boolConf.setNestedCCs(ccs);
	boolConf.setBoolOperator(BoolOperator.AND);
	return boolConf;
    }
	...
	boolConf.setNestedCCs(ccs);
	boolConf.setBoolOperator(BoolOperator.OR);
	return boolConf;
    }
	...
	//get Slot Name
	JamochaValue slotName = (JamochaValue) node.jjtGetChild(1).jjtAccept(this, data);	
