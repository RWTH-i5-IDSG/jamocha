	...
	throws EvaluationException {
		...
		if (sd.is(JamochaType.STRING)){
			...
			service = service.substring(1, service.length()-1); // Delete the '' around the JamochaString
				...
				agent.send(message);
				String rule;
				rule ="(defrule suche_message ?message <- (agent-message (conversation-id "+message.getConversationId()+") ) => (agent-unregister ?message))";
				...
				if (helper != "") {
					...
					String rule ="(defrule suche_message ?message <- (agent-message (conversation-id "+message.getConversationId()+") ) => (agent-unregister ?message))";
					String delrule = "(undefrule suche_message)";
					channel = agent.getEngine().getMessageRouter().openChannel(
							agent.getProperties().getProperty("agent.name", "Agent"));
	...
	private boolean checkanswer (Fact answer, Rete engine)
		...
		if (answer.getSlotValue("performative").toString().contains("inform")) {
			...
			if (newcontent.contains("not-registered")){
				engine.writeMessage("Sorry, that agent is not registered with the df!", output);
