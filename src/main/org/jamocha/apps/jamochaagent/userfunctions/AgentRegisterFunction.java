...
import java.util.*;
//import org.jamocha.rete.exception.*;
...
public class AgentRegisterFunction extends AbstractFunction {
	...
	public String output = "t"; //"JamochaGui_1"; //output when receiving a message
	 ...
	 * Probleme:
	 * '' um Strings
	 * Antworten nach dem Aufruf in der rulenur noch im log
	 */
				...
				if (params[0].getValue(engine).is(JamochaType.FACT)) {
					...
					if (checkanswer(answer, engine)) {
						/*if (output.contains("JamochaGui"))
							channel.executeCommand("(printout "+ output +" \"true\" crlf)");*/
						engine.writeMessage("true", output);
						return JamochaValue.TRUE;	
					}
					else {
						/*if (output.contains("JamochaGui"))
							channel.executeCommand("(printout "+ output +" false crlf)");*/
	...
	throws EvaluationException {
		...
		/*String deftemplate ="(deftemplate agent-message" +
		"(slot sender)" +
		"(multislot receiver)" +
		"(multislot reply-to)" +
		"(slot performative)" +
		...
		"(slot incoming (type BOOLEAN))" +
		"(slot processed (type BOOLEAN))" +
		"(slot is-template (type BOOLEAN))" +
		")";
		String undeftemplate ="(undeftemplate agent-message)";
*/
		if (sd.is(JamochaType.STRING)){
			...
			service = service.substring(1, service.length()-1); // Delete the '' around the JamochaString
				...
				String rule;
				rule ="(defrule suche_message ?message <- (agent-message (conversation-id "+message.getConversationId()+") ) => (agent-register ?message))";
				...
				if (helper != "") {
					...
					agent.send(message);
					String rule ="(defrule suche_message ?message <- (agent-message (conversation-id "+message.getConversationId()+") ) => (agent-register ?message))";
					String delrule = "(undefrule suche_message)";
					channel = agent.getEngine().getMessageRouter().openChannel(
							agent.getProperties().getProperty("agent.name", "Agent"));
	...
	private boolean checkanswer (Fact answer, Rete engine)
		...
		if (answer.getSlotValue("performative").toString().contains("inform")) {
			...
			if ( (newcontent.contains("done")) ){ 
				...
				//channel.executeCommand("(printout "+ output +" \"Got wrong content at the inform message but the right ConversationID\")");
				engine.writeMessage("Got wrong content at the inform message but the right ConversationID", output);
				return false;
			}
		}
		else {
			...
			if (newcontent.contains("already-registered")){
				engine.writeMessage("Sorry, that agent is already registered with the df!", output);
				//channel.executeCommand ("(printout "+ output +"  \"Sorry, that agent is already registered with the df!\")");
				return false;
			}
