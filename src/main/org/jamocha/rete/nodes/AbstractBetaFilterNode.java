 ...
 * This class provides an abstract node type, which has two inputs and
 * a set of join filters.
 */
public abstract class AbstractBetaFilterNode extends TwoInputNode {
	...
	public AbstractBetaFilterNode(int id, WorkingMemory memory, ReteNet net, JoinFilter[] filters) {
		super(id, memory, net);
		this.filters = filters;
	}
	
	/**
	 * returns true, iff the given alpha- and beta-wme combination is accepted by the filters
	 * @param alpha
	 * @param beta
	 * @return
	...
	protected boolean applyFilters(WorkingMemoryElement alpha, WorkingMemoryElement beta) throws JoinFilterException, EvaluationException {
		for (JoinFilter f:getFilters()) {
			if (!f.evaluate(alpha.getFirstFact(), beta.getFactTuple(), net.getEngine())) {
				return false;
			}
		}
		return true;
	 ...
	 */
	protected abstract void addBeta(WorkingMemoryElement newElem) throws JoinFilterException, EvaluationException, NodeException;

	/**
	 * this method is called, when a new alpha wme is added.
	 */
	protected abstract void addAlpha(WorkingMemoryElement newElem) throws JoinFilterException, EvaluationException, NodeException;
	 ...
	 */
	protected abstract void removeBeta(WorkingMemoryElement oldElem) throws JoinFilterException, EvaluationException, NodeException;

	/**
	 * this method is called, when a beta wme is removed.
	 */
	protected abstract void removeAlpha(WorkingMemoryElement oldElem) throws JoinFilterException, EvaluationException, NodeException;
	...
	@Deprecated
	/**
	 * this method should never be called! Only exception is the usage
	 * from the old SFRuleCompiler. New rule compilers must determine
	 * the filter array before and use the constructor argument for setting it
	 */
	...
	public void addFilter(JoinFilter filter) {
		JoinFilter[] arr = new JoinFilter[filters.length + 1];
		System.arraycopy(filter, 0, arr, 0, filters.length);
		arr[filters.length]	= filter;
		setFilter(arr);
	}
