 ...
 * an abstract node with two inputs: an alpha- and a beta- input.
 * it is a subclass from OneInputNode, since a node with two inputs
 * can be seen as a node with one input and an additional one ;)
 */
public abstract class TwoInputNode extends OneInputNode {
	...
	protected void plugBetaParent(Node node) throws InvalidOperationException, NodeException {
		if (betaInput != null) throw new InvalidOperationException("there already is a beta input");
		betaInput = node;
		node.addChild(this);
	}
	...
	protected Node registerParent(Node n) throws NodeException {
		if (n.outputsBeta()) {
			...
			if (alphaInput == null) {
				...
				LeftInputAdaptorNode lia = new LeftInputAdaptorNode(net.nextNodeId(), workingMemory, net);
				lia.addChild(this);
