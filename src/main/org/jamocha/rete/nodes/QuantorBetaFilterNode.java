 ...
 * a QuantorBetaFilterNode is a BetaFilterNode, which has a slightly different
 * output. 
 * for each beta-input, it determines, whether there exists at least one alpha-input,
 * which is accepted by the given filters. if so, it outputs this beta-wme _only once_.
 * this node can also be negated. a negated QuantorBetaFilterNode outputs a beta-wme, iff
 * there is _no_ alpha-input accepted by the given filters.
 */
public class QuantorBetaFilterNode extends SimpleBetaFilterNode {
	...
	public QuantorBetaFilterNode(int id, WorkingMemory memory, ReteNet net, JoinFilter[] filter, boolean negated) {
	...
	protected void itExists(WorkingMemoryElement w) throws NodeException {
		if (negated) removeAndPropagate(w); else addAndPropagate(w);
	}
	
	protected void itDoesntExist(WorkingMemoryElement w) throws NodeException {
		if (negated) addAndPropagate(w); else removeAndPropagate(w);
	}
	...
	protected void evAlpha(WorkingMemoryElement elem, int direction) throws JoinFilterException, EvaluationException, NodeException {
		...
		for (WorkingMemoryElement wme : beta() ){
			boolean match = applyFilters(elem, wme);
			if (match) {
				int now = counter.add(wme, direction);
				assert (now >= 0);
				if (direction>0 && now == 1) {
					itExists( wme.getFactTuple().appendFact(net.engine.getInitialFact()) );
				} else if (direction<0 && now == 0) {
					itDoesntExist( wme.getFactTuple().appendFact(net.engine.getInitialFact()) );
				}
			}
		}
	}
	...
	protected void addBeta(WorkingMemoryElement newElem) throws JoinFilterException, EvaluationException, NodeException {
		...
		for (WorkingMemoryElement wme : alpha() ) {
			if (applyFilters(wme, newElem)) cn++; 
		}
		if (cn > 0) {
			counter.set(newElem, cn);
			itExists( newElem.getFactTuple().appendFact(net.engine.getInitialFact()) );
		} else {
			itDoesntExist( newElem.getFactTuple().appendFact(net.engine.getInitialFact()) );
		}
	}
	...
	protected void removeBeta(WorkingMemoryElement oldElem)	throws JoinFilterException, EvaluationException, NodeException {
		removeAndPropagate( oldElem.getFactTuple().appendFact(net.engine.getInitialFact()) );
