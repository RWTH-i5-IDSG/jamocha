 ...
 * 
 * This is a concrete Shape-Implementation for an rectangle with a
...
public class Rectangle extends Shape {
	...
	/**
	 * @param bgcolor
	 *            The Fill-Color the ellipse should get
	 * @param bordercolor
	 *            The Border-Color the ellipse should get
	 * @param x
	 ...
	 */
	public Rectangle(Color bgcolor, Color bordercolor, int x, int y, int width,
	...
	/**
	 * Draws the rectangle. The draw-position is translated by
	 * (-offsetX,-offsetY).
	 * 
	 * @param canvas
	 *            The canvas to draw the arrow on
	 * @param offsetX
	 *            Translation-Vector's negative x-component
	 * @param offsetY
	 *            Translation-Vector's negative y-component
	 */
	...
	/**
	 * Draws the rectangle. The draw-position is translated by
	 * (-offsetX,-offsetY) and scaled by (factorX,factorY).
	 * 
	 * @param canvas
	 *            The canvas to draw the arrow on
	 * @param offsetX
	 ...
	 *            Translation-Vector's negative y-component
	 * @param factorX
	 *            Scaling-Vector*s x-component
	 * @param factorY
	 *            Scaling-Vector*s y-component
	 */
	public void draw(Graphics2D canvas, int offsetX, int offsetY,
		...
		if (height > 10) {
			...
			canvas.drawString(text, (int) textpos.getX() + x, (int) textpos
					.getY()
					+ y);
		}
	...
	public Point calculateIntersection(double angle) {
		...
		if (angle > angleTopLeft || angle < angleBottomLeft) {
			...
			result.x = x;
			result.y = (int) Math.round(-Math.tan(alpha) * width * 0.5 + y
					+ height * 0.5);
		} else if (angle > angleTopRight) {
			// top
			double alpha = Math.PI * 0.5 - angle;
			result.x = (int) Math.round(Math.tan(alpha) * height * 0.5 + x
					+ width * 0.5);
			result.y = y;
