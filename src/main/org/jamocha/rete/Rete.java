...
public class Rete implements PropertyChangeListener, CompilerListener, Serializable {
    ...
    /**
         * this is the HashMap for all functions. This means all function names
         * are unique.
         */
    protected Map functions = CollectionsFactory.localMap();

    /**
         * We keep a map between the object instance and the corresponding
         * shadown fact. If an object is added as static, it is added to this
         * map. When the rule engine is notified of changes, it will check this
         * list. If the object instance is in this list, we ignore it.
         */
    protected Map staticFacts = CollectionsFactory.localMap();

    /**
         * We keep a map of the dynamic object instances. When the rule engine
         * is notified
         */
    ...
    /**
         ...
         * already exists in the working memory. this is only used for deffacts
         * and not for objects
         */
    ...
    /**
         * clear the deffacts from the working memory. This does not include
         * facts asserted using assertObject.
         */
    ...
    /**
         * this is useful for debugging purposes. clips allows the user to fire
         * 1 rule at a time.
         * 
         * @param count
         * @return
    ...
    /**
         ...
         * will be fired. Activations in other modules will not be fired until
         * the focus is changed to it.
         * 
         * @return
         */
    ...
    /**
         * find the template starting with other modules and ending with the
         * main module.
         * 
         * @param name
         * @return
    ...
    /**
         * Users can write query adapters to execute an external query. The
         * method will find all Query adapters for a given Template. There can
         * be zero or more adapter registered for a given template. If the
         * template hasn't been defined, the method return null.
         * 
         * @param template
         * @return
         */
    ...
    /**
         * Implementation will lookup the defclass for a given object by using
    ...
    /**
         * method is specifically for templates that are declared in the shell
         * and do not have a corresponding java class.
         * 
         * @param temp
         */
    ...
    /**
         * To explicitly deploy a custom function, call the method with an
    ...
    /**
         * Method will create in instance of the FunctionGroup class and load
         * the functions.
         * 
         * @param name
         */
    ...
    /**
         * pass a filename to load the rules. The implementation uses
         * BatchFunction to load the file.
         * 
         * @param filename
    ...
    /**
         * Returns a list of the function groups. If a function is not in a
         * group, get the complete list of functions using getAllFunctions
         * instead.
         * 
         * @return
         */
    ...
    /**
         ...
         * implementation will check to see if the name of the variable begins
         * and ends with "*". If it does, it will declare it as a defglobal.
         * Otherwise, it will try to add it to the rule being fired. Note: might
         * need to have add one for shell variables later.
         * 
         * @param key
         * @param value
         */
    ...
    /**
         * Rete class contains a list of items that can be watched. Call the
         * method with one of the four types:<br/> activations<br/> all<br/>
         * facts<br/> rules<br/>
         * 
         * @param type
         */
    ...
    /**
         * To turn on profiling, call the method with the appropriate parameter.
    ...
    /**
         * To turn off profiling, call the method with the appropriate
         * parameter. The parameters are defined in Rete class as static int
         * values.
         * 
         * @param type
         */
    ...
    /**
         * return a list of all the facts including deffacts and shadow of
         * objects
         * 
         * @return
         */
    ...
    /**
         * It is up to spool function to make sure it removes the printer writer
	...
	router.postMessageEvent(new MessageEvent(MessageEvent.ENGINE, msg, "t".equals(output) ? router
		.getCurrentChannelId() : output));
    ...
    /**
         * The method will print out the node. It is up to the method to check
         * if pretty printer is true and call the appropriate node method to get
         * the string. TODO - need to implement this
         * 
         * @param node
         */
	...
	if (debug) {
	    System.out.println("\"assert at nodeid=" + node.nodeID + " - " + node.toString().replaceAll("\"", "'")
		    + ":: with fact -" + fact.toFactString().replaceAll("\"", "'") + "::\"");
	}
    ...
    /**
         * The current implementation of assertObject is simple, but flexible.
         * This version is not multi-threaded and doesn't use an event queue.
         * Later on a multi-threaded version will be written which overrides the
         * base implementation. If the user passes a specific template name, the
         * engine will attempt to only propogate the fact down that template. if
         * no template name is given, the engine will propogate the fact down
         * all input nodes, including parent templates.
         * 
         * @param data
         * @param template
         * @param statc
    ...
    /**
         * Modify will call retract with the old fact, followed by updating the
         * fact instance and asserting the fact.
         * 
         * @param data
         */
    ...
    /**
         * Assert with profiling will use the ProfileStats class to measure the
         * time to assert facts
         * 
         * @param fact
         * @throws AssertException
         */
    ...
    /**
         * retract by fact id is slower than retracting by the deffact instance.
         * the method will find the fact and then call retractFact(Deffact)
         * 
         * @param id
         */
    ...
    /**
         * Modify retracts the old fact and asserts the new fact. Unlike
         * assertFact, modifyFact will not check to see if the fact already
         * exists. This is because the old fact would already be unique.
         * 
         * @param old
         * @param newfact
         */
    ...
    /**
         * Method will retract the objects and re-assert them. It does not reset
         * the deffacts.
         */
    ...
    /**
         * Method will retract all the deffacts and then re-assert them. Reset
         * does not reset the objects. To reset both the facts and objects, call
         * resetAll. resetFacts handles deffacts which are not derived from
         * objects.
         */
    ...
    /**
         * The implementation will look in the current module in focus. If it
         * isn't found, it will search the other modules. The last module it
         * checks should be the main module.
         * 
         * @param data
         * @param id
         * @return
    ...
    /**
         * This is temporary, it should be replaced with something like the
         * current factHandleFactory().newFactHandle()
         * 
         * @return
         */
