 ...
 * @author Peter Lin
 *
 * BasicRuleCompiler is a simple implementation. It's for research purposes
                ...
                if (cnstr instanceof LiteralConstraint) {
                    ...
                    if (util.getBinding(bc.getVariableName()) == null) {
                        ...
                        if (bc.getIsObjectBinding()) {
                            ...
                            bind.setVarName( bc.getVariableName() );
                            bind.setLeftRow(position);
                            bind.setLeftIndex( templ.getSlot(bc.getName()).getId() );
                            bind.setRowDeclared(position);
                            bc.setFirstDeclaration(true);
                            util.addBinding(bc.getVariableName(),bind);
                        }
                    } else {
                        // if the binding already exists, we need do nothing currently
                    }
                    ...
                    if (!pc.isPredicateJoin()) {
                        if (ConversionUtils.isPredicateOperatorCode(pc.getFunctionName())) {
                            int oprCode = ConversionUtils.getOperatorCode(pc.getFunctionName());
                            Slot sl = (Slot)templ.getSlot(pc.getName()).clone();
                            Object sval = 
                            	ConversionUtils.convert(sl.getValueType(),cnstr.getValue());
                            sl.value = sval;
    ...
    throws AssertException
    {
        ...
        if (conds.length > 1) {
            ...
            if (prev instanceof ObjectCondition) {
                ...
                // if the condition doesn't have any nodes, we add the LeftInputAdapter
                // to the ObjectTypeNode, otherwise we add it to the last AlphaNode
            ...
            for (int idx=1; idx < conds.length; idx++) {
                ...
                if (cdt instanceof TestCondition) {
                    ...
                    Deftemplate tmpl = exc.getObjectCondition().getDeftemplate();
                    ...
                    for (int idz=0; idz < binds.length; idz++) {
                    	...
                    	if (cst instanceof BoundConstraint) {
                    		...
                    		if (pc.getValue() instanceof BoundParam) {
                        		...
                        		int op = ConversionUtils.getOperatorCode(
                        				pc.getFunctionName());
                                ...
                                binds[idz].setRightIndex(rinx);
                    		} else if (pc.getValue() instanceof ValueParam) {
                    			// do nothing for now.
                    		}
                    	}
                    }
                    ...
                    if (cdt.hasBindings()) {
                        ...
                        // now add the join node to the last nodes of the current
        	...
        	if (conds[0] instanceof ObjectCondition) {
        		...
        		if (oc.getNegated()) {
        			...
        			// to the LIANode.
        			ObjectTypeNode otn = (ObjectTypeNode)this.inputnodes.get(
        					engine.initFact);
