...
public class EngineSettingsPanel extends AbstractSettingsPanel implements ActionListener {
	...
	private JCheckBox shareNodesCheckBox;
	...
	public EngineSettingsPanel(JamochaGui gui) {
		...
		// shareNodes
		addLabel(generalSettingsPanel, new JLabel("Share Nodes:"), gridbag, c, 9);
	...
	public void refresh() {
		...
		// check boxes:
		evaluationCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_EVALUATION));
		profileAssertCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_ASSERT));
		profileRetractCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_RETRACT));
		profileFireCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_FIRE));
		profileAddActivationCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_ADD_ACTIVATION));
		profileRemoveActivationCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_REMOVE_ACTIVATION));
		watchActivationsCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_ACTIVATIONS));
		watchFactsCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_FACTS));
		watchRulesCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_RULES));
		shareNodesCheckBox.setSelected(settings.getBoolean(SettingsConstants.ENGINE_NET_SETTINGS_SHARE_NODES));

		strategySelectorMain.setSelectedItem(settings.getString(SettingsConstants.ENGINE_STRATEGY_SETTINGS_STRATEGY_MAIN));
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == evaluationCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_EVALUATION, evaluationCheckBox.isSelected());
		} else if (event.getSource() == profileAssertCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_ASSERT, profileAssertCheckBox.isSelected());
		} else if (event.getSource() == profileRetractCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_RETRACT, profileRetractCheckBox.isSelected());
		} else if (event.getSource() == profileFireCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_FIRE, profileFireCheckBox.isSelected());
		} else if (event.getSource() == profileAddActivationCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_ADD_ACTIVATION, profileAddActivationCheckBox.isSelected());
		} else if (event.getSource() == profileRemoveActivationCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_REMOVE_ACTIVATION, profileRemoveActivationCheckBox.isSelected());
		} else if (event.getSource() == watchActivationsCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_ACTIVATIONS, watchActivationsCheckBox.isSelected());
		} else if (event.getSource() == watchFactsCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_FACTS, watchFactsCheckBox.isSelected());
		} else if (event.getSource() == watchRulesCheckBox) {
			settings.set(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_RULES, watchRulesCheckBox.isSelected());
		} else if (event.getSource() == shareNodesCheckBox) {
			settings.set(SettingsConstants.ENGINE_NET_SETTINGS_SHARE_NODES, shareNodesCheckBox.isSelected());
		}
		// combo boxes:
		else if (event.getSource().equals(strategySelectorMain)) {
			if (strategySelectorMain.getSelectedItem() != null) {
				String strategyName = strategySelectorMain.getSelectedItem().toString();
				settings.set(SettingsConstants.ENGINE_STRATEGY_SETTINGS_STRATEGY_MAIN, strategyName);
			}
	...
	private void initStrategySelectorMain() {
		...
		String currName = settings.getString(SettingsConstants.ENGINE_STRATEGY_SETTINGS_STRATEGY_MAIN);
		Set<String> strategies = ConflictResolutionStrategy.getStrategies();
		for (String strategyName : strategies) {
	...
	public void setDefaults() {
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_EVALUATION);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_ASSERT);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_RETRACT);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_FIRE);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_ADD_ACTIVATION);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_PROFILE_REMOVE_ACTIVATION);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_ACTIVATIONS);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_FACTS);
		settings.toDefault(SettingsConstants.ENGINE_GENERAL_SETTINGS_WATCH_RULES);
		settings.toDefault(SettingsConstants.ENGINE_NET_SETTINGS_SHARE_NODES);
		settings.toDefault(SettingsConstants.ENGINE_STRATEGY_SETTINGS_STRATEGY_MAIN);
		refresh();
	}
}
