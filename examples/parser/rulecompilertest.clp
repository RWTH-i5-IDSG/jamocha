...
(defmodule WURSTMODUL)

(deftemplate wurst (slot name))

(assert (wurst (name bratwurst)))
...
(defmodule SALATMODUL)

(deftemplate salat (slot name))
...
(set-focus WURSTMODUL)
(fire)

(set-focus MAIN)
(fire)

(set-focus SALATMODUL)
(assert (salat (name kartoffelsalat)))
(fire)
